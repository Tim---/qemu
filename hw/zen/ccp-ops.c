...
{
    address_space_read(&s->dma_as, addr, MEMTXATTRS_UNSPECIFIED,
                       buf, len);
}
...
{
    /* TODO: unsafe memcpy ! */
    memcpy(buf, &s->sb[addr], len);
}


static void write_to_system(CcpState *s, hwaddr addr, void *buf, hwaddr len)
{
    address_space_write(&s->dma_as, addr, MEMTXATTRS_UNSPECIFIED,
                        buf, len);
}
...
int ccp_op_aes(CcpState *s, struct ccp5_desc * desc)
{
    ...
    trace_ccp_aes(aes_type->name, aes_mode->name,
                  src_addr.addr, memtype_to_str(src_addr.type),
                  dst_addr.addr, memtype_to_str(dst_addr.type),
...
int ccp_op_passthru(CcpState *s, struct ccp5_desc * desc)
{
    ...
    trace_ccp_passthru(src_addr.addr, memtype_to_str(src_addr.type),
                       dst_addr.addr, memtype_to_str(dst_addr.type),
                       length);
...
static void ccp_sha512_update(union sha_ctx *ctx, size_t length,
                              const uint8_t *data)
{
    sha512_update(&ctx->sha512_ctx, length, data);
}

static void ccp_sha512_digest(union sha_ctx *ctx, size_t length,
                              uint8_t *digest)
{
...
int ccp_op_sha(CcpState *s, struct ccp5_desc * desc)
{
    ...
    trace_ccp_sha(algo->name,
                  src_addr.addr, memtype_to_str(src_addr.type),
                  lsb_addr.addr, memtype_to_str(lsb_addr.type),
                  length);
...
int ccp_op_rsa(CcpState *s, struct ccp5_desc * desc)
{
    ...
    trace_ccp_rsa(src.addr, memtype_to_str(src.type),
...
int ccp_op_zlib(CcpState *s, struct ccp5_desc * desc)
{
    ...
    trace_ccp_zlib(src.addr, memtype_to_str(src.type),
                   dst.addr, memtype_to_str(dst.type), length);
