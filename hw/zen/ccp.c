...
static void ccp_run(CcpState *s, ccp_queue_t *queue)
{
    ...
    while (head_idx != tail_idx) {
        ...
        MemTxResult mem_res = address_space_read(&s->dma_as, queue->head, MEMTXATTRS_UNSPECIFIED, &desc, sizeof(desc));
...
static void ccp_realize(DeviceState *dev, Error **errp)
{
    ...
    address_space_init(&s->dma_as, s->system_memory, "ccp-as");
...
static void ccp_class_init(ObjectClass *class, void *data)
{
    ...
    dc->desc = "PSP CCP";
    device_class_set_props(dc, ccp_props);
}
