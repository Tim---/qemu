...
static MemoryRegion *create_ram(const char *name, MemoryRegion *mem, hwaddr addr, uint64_t size)
{
...
static MemoryRegion *create_rom(const char *name, MemoryRegion *mem, hwaddr addr, uint64_t size)
{
    MemoryRegion *region = g_new0(MemoryRegion, 1);
...
static void pcie_x86_mmcfg_map(PspMachineState *pms, PCIExpressHost *e, hwaddr addr,
                         uint32_t size)
{
...
static void create_smn_dirt(PspMachineState *pms)
{
    ...
    stub_create("cores-bitmap", &pms->smn_region,   0x0005a870, 4, 0x00000001); // bitmap of cores present

    stub_create("fuses0",         &pms->smn_region,   0x0005d1f8, 4, 0); // some fuses
    stub_create("fuses1",         &pms->smn_region,   0x0005d1fc, 4, 0); // some fuses

    // 0x03000000 -> 0x04000000 looks full of SMU
    create_ram("smu-ram",       &pms->smn_region,   0x03c00000, 0x00040000);
    stub_create("fusestraps",   &pms->smn_region,   0x03e10024, 4, 1); // MP2_RSMU_FUSESTRAPS
...
static void psp_machine_init(MachineState *machine)
{
    ...
    memory_region_init(&pms->smn_region, OBJECT(pms), "smn-region", 0x100000000);
    create_unimplemented_device_custom("smn-unimpl", &pms->smn_region, 0, 0x100000000, true);

    // X86 regions !
    memory_region_init(&pms->x86_full_region, OBJECT(pms), "x86-full-region", 0x1000000000000UL);
    create_unimplemented_device_custom("x86-full-unimpl", &pms->x86_full_region, 0, 0x1000000000000UL, false);

    memory_region_init(&pms->x86_region_fffd, OBJECT(pms), "x86-fffd-region", 0x100000000UL);
    create_unimplemented_device_custom("x86-fffd-unimpl", &pms->x86_region_fffd, 0, 0x100000000UL, false);
    memory_region_add_subregion(&pms->x86_full_region, 0xfffd00000000UL, &pms->x86_region_fffd);

    memory_region_init(&pms->x86_region_fffe, OBJECT(pms), "x86-fffe-region", 0x100000000UL);
    create_unimplemented_device_custom("x86-fffe-unimpl", &pms->x86_region_fffe, 0, 0x100000000UL, false);
    memory_region_add_subregion(&pms->x86_full_region, 0xfffe00000000UL, &pms->x86_region_fffe);

    memory_region_init(&pms->x86_region_0000, OBJECT(pms), "x86-0000-region", 0x100000000UL);
    create_unimplemented_device_custom("x86-0000-unimpl", &pms->x86_region_0000, 0, 0x100000000UL, false);
    memory_region_add_subregion(&pms->x86_full_region, 0x000000000000UL, &pms->x86_region_0000);

    // PSP SOC
    object_initialize_child(OBJECT(machine), "soc", &pms->soc, TYPE_PSP_SOC);
    object_property_set_link(OBJECT(&pms->soc), "ram", OBJECT(machine->ram), &error_abort);
    object_property_set_link(OBJECT(&pms->soc), "smn-memory", OBJECT(&pms->smn_region), &error_abort);
    object_property_set_link(OBJECT(&pms->soc), "x86-memory", OBJECT(&pms->x86_full_region), &error_abort);
    object_property_set_str(OBJECT(&pms->soc), "cpu-type", machine->cpu_type, &error_abort);
    qdev_realize(DEVICE(&pms->soc), NULL, &error_abort);

    // FCH ACPI
    object_initialize_child(OBJECT(machine), "fch-acpi", &pms->fch_acpi, TYPE_FCH_ACPI);
    if (!sysbus_realize(SYS_BUS_DEVICE(&pms->fch_acpi), &error_fatal))
        return;
    memory_region_add_subregion(&pms->x86_region_0000, ACPI_MMIO_ADDR, sysbus_mmio_get_region(SYS_BUS_DEVICE(&pms->fch_acpi), 0));
    memory_region_add_subregion(&pms->smn_region, 0x02d01000, sysbus_mmio_get_region(SYS_BUS_DEVICE(&pms->fch_acpi), 1));
    ...
    if (!sysbus_realize(SYS_BUS_DEVICE(&pms->fch_spi), &error_fatal))
        return;
    memory_region_add_subregion(&pms->smn_region, 0x02dc4000, sysbus_mmio_get_region(SYS_BUS_DEVICE(&pms->fch_spi), 0));

    // SPI ROM
    MemoryRegion *rom_region = create_rom("rom", &pms->smn_region, 0x0A000000, 0x01000000);
    ...
    if (!sysbus_realize(SYS_BUS_DEVICE(&pms->smu), &error_fatal))
        return;
    memory_region_add_subregion(&pms->smn_region, 0x03b10000, sysbus_mmio_get_region(SYS_BUS_DEVICE(&pms->smu), 0));

    // UMC
    object_initialize_child(OBJECT(machine), "umc0", &pms->umc0, TYPE_PSP_UMC);
    if (!sysbus_realize(SYS_BUS_DEVICE(&pms->umc0), &error_fatal))
        return;
    memory_region_add_subregion(&pms->smn_region, 0x050000, sysbus_mmio_get_region(SYS_BUS_DEVICE(&pms->umc0), 0));

    object_initialize_child(OBJECT(machine), "umc1", &pms->umc1, TYPE_PSP_UMC);
    if (!sysbus_realize(SYS_BUS_DEVICE(&pms->umc1), &error_fatal))
        return;
    memory_region_add_subregion(&pms->smn_region, 0x150000, sysbus_mmio_get_region(SYS_BUS_DEVICE(&pms->umc1), 0));

    // PCI host bridge
    PCIHostState *phb = PCI_HOST_BRIDGE(create_pcie_root(pms));

    // FCH LPC (including serial port)
    PCIDevice *lpc = pci_create_simple_multifunction(phb->bus, PCI_DEVFN(0x14, 0x3), true, TYPE_FCH_LPC);
    assert(lpc);

    // Is the LPC the owner of the IO space ?
    memory_region_add_subregion(&pms->x86_region_fffd, X86_IO_BASE, get_system_io()); // TODO: we don't get unimplemented RW
...
#ifdef BOOT_SECURED_OS
    //psp_load_secured_os(machine->firmware, 0xbc0a0000);
